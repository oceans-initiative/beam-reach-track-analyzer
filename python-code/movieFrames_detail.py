...
def makeMovieFrames(whale, boats, idx, plt):
  ...
  xCenter = whale.xMod[idx]
  yCenter = whale.yMod[idx]
 
  delta = 200
  ax.set_xlim(xCenter-delta,xCenter+delta)
  ax.set_ylim(yCenter-delta,yCenter+delta)
  # xScale = [xCenter - 50, xCenter + 50]
  # yScale = [yCenter+delta - 0.15*delta, yCenter+delta - 0.15*delta]
  # plt.plot(xScale,yScale,linewidth=2,color='black')
  # plt.text(xCenter-30, yCenter+delta - 0.095*delta, '100 m' )
  
  delta = 25
  xScale = [xCenter - 5, xCenter + 5]
  yScale = [yCenter+delta - 0.15*delta, yCenter+delta - 0.15*delta]
  plt.plot(xScale,yScale,linewidth=2,color='black')
  plt.text(xCenter-3, yCenter+delta - 0.095*delta, '10 m' )  
...
for idx in range(0, Nframes, 1):  ###  Nframes//25):
  makeMovieFrames(whale,boats, idx, plt)   
#  plt.show()  
  plotDirectory = "/home/val/Documents/NFWF_Files/2020_Analysis/framesDetail"
  filename = "%s/img%03d.png" % (plotDirectory, idx)
  plt.gca().axes.get_xaxis().set_visible(False)
  plt.gca().axes.get_yaxis().set_visible(False)
  plt.savefig(filename)
  plt.show(block=False)  # force plot to show on screen  
